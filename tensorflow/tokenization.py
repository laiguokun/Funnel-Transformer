#! coding=utf-8
"""Tokenization with sentence piece or word piece."""
...
def preprocess_text(inputs, lower=False, remove_space=True, keep_accents=False):
  ...
  outputs = outputs.replace(u"``", u'"').replace(u"''", u'"')
  outputs = outputs.replace(u'”', u'"').replace(u'“', u'"')
  outputs = outputs.replace(u"‘", u"'").replace(u"’", u"'")
...
def encode_pieces(sp_model, text, return_unicode=True, sample=False):
  ...
  # Note: in some systems, sentencepiece only accepts str for py2
  ...
  # Note: convert back to unicode for py2
...
def get_tokenizer():
  if FLAGS.tokenizer_type == "sent_piece":
    return SPMTokenizer(FLAGS.tokenizer_path, FLAGS.uncased)
  elif FLAGS.tokenizer_type == "word_piece":
    return FullTokenizer(FLAGS.tokenizer_path, FLAGS.uncased)
  else:
